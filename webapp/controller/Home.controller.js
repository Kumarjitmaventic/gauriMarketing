sap.ui.define(
    [
        ...
        "sap/m/MessageToast",
        "sap/m/MessageBox",
        "marketingcampaignreq/zcrmktmarketingcampaign/utils/formatter",
    ...
    function (
        ...
        return Controller.extend(
            "marketingcampaignreq.zcrmktmarketingcampaign.controller.Home",
            {
                formatter: formatter,
            ...
            */
                ...
                fetchRequestDetailsAndUpdateModel: async function (requestId) {
                    ...
                    if (requestId) {
                        ...
                        if (requestDetails) {
                            ...
                            uiModel.setProperty(
                                "/backup",
                                JSON.parse(JSON.stringify(requestDetails))
                            );
                            uiModel.setProperty(
                                "/ui/requestStatus",
                            ...
                            // for draft mode
                            if (
                                requestDetails.Status == "DRF" ||
                                (requestDetails.Status == "CFR" &&
                                    uiModelPropertices.role == "Requester")
                            )
                            ...
                            if (
                                requestDetails.Status === "APR" ||
                                requestDetails.Status === "REJ"
                            )
                                requestModel.setProperty(
                                    "/updatedAt",
                ...
                onUploadFiles() {
                    if (!this._oDialog) {
                        this._oDialog = sap.ui.xmlfragment(
                            this.getView().getId(),
                            "marketingcampaignreq.zcrmktmarketingcampaign.view.fragments.uploadFile",
                            this
                        );
                ...
                onPressActions: async function (oEvent) {
                    let actionText = oEvent.getSource().getText();
                ...
                //                 height: "120px",
                //                 maxLength: 255,
                //                 placeholder: `Decision note  (${actionType == "Approve"? "Optional" : "Required" })`,
                //             }),
                //         ],
                ...
                //                 actionType == "Approve"
                //                     ? ButtonType.Accept
                //                     : actionType == "Reject" ? ButtonType.Reject : ButtonType.Attention,
                //             text: actionType,
                ...
                onActionConfirmDialogBox: function (actionType) {
                    ...
                    const actionTypeConfigMap = {
                        Approve: {
                            btnType: ButtonType.Accept,
                            status: "Approve",
                            isActionNoteReq: false,
                            message: {
                                success: "Request successfully Approved.",
                                faild: "Error while approved the Request.",
                            },
                        },
                        Reject: {
                            btnType: ButtonType.Reject,
                            status: "Reject",
                            isActionNoteReq: true,
                            message: {
                                success: "Request successfully Rejected.",
                                faild: "Error while reject the request.",
                            },
                        },
                        "Clarification Required": {
                            btnType: ButtonType.Attention,
                            status: "Clar Rqrd",
                            isActionNoteReq: true,
                            message: {
                                success:
                                    "Clarification requested successfully.",
                                faild: "Error while Clarification request.",
                            },
                        },
                        "Clarification Provider": {
                            btnType: ButtonType.Emphasized,
                            status: "Clar Pro",
                            isActionNoteReq: true,
                            message: {
                                success: "Clarification provided successfully.",
                                faild: "Error while clarification providation.",
                            },
                        },
                    };
                    let onApproverActionConfirmDialog = new sap.m.Dialog({
                        ...
                        beginButton: new Button({
                            type: actionTypeConfigMap[actionType].btnType,
                            text: actionType,
                            press: function (oEvent) {
                                let oTextArea = sap.ui
                                    .getCore()
                                    .byId("confirmationNote");
                                let decisionNote = oTextArea.getValue().trim();

                                if (
                                    actionTypeConfigMap[actionType]
                                        .isActionNoteReq &&
                                    !decisionNote
                                ) {
                                    oTextArea.setValueState(
                                ...
                                this.onApproverAction(
                                    oEvent,
                                    actionTypeConfigMap[actionType].status,
                                    decisionNote,
                                    actionTypeConfigMap[actionType].message
                                );
                                onApproverActionConfirmDialog.close();
                            }.bind(this),
                        }),
                        endButton: new Button({
                            ...
                            press: function () {
                                ...
                                let oTextArea = sap.ui
                                    .getCore()
                                    .byId("confirmationNote");
                                oTextArea.setValue(""); // clear message
                                oTextArea.setValueState(
                                    sap.ui.core.ValueState.None
                                );
                            },
                ...
                onAddFile(oEvent) {
                    ...
                    const description =
                        oDocModel.getProperty("/Doc/Description");
                ...
                onPressSave: async function () {
                    ...
                    const payload = {
                        ...
                        Status: uiModelProperties.ui.requestStatus,
                    };
                ...
                uploadDocuments: async function (requestId) {
                    ...
                    backup?.Document?.results.forEach((item) => {
                        ...
                        if (
                            ...
                            deletedItemPayload.DocList.push({
                                DocId: item.DocId,
                            });
                    });
                    if (deletedItemPayload?.DocList?.length > 0)
                        uploadDocPayloads.push(
                            that._OdataSyncPostReqSave(
                                "/DeletionDocumentSet",
                                oModel,
                                deletedItemPayload
                            )
